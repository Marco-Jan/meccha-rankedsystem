...
namespace MecchaRanked
{
    ...
    static class Sprache
    {
        ...
        static readonly Dictionary<string, string[]> W = new Dictionary<string, string[]>
        {
            ...
            { "Erst am Ende der Runde drücken – die Punkte laufen bis zuletzt weiter. Dabei auf einen ruhigen, kontrastreichen Hintergrund schauen: Himmel oder eine Wand statt buntem Boden.",
              new[] { "Press only at the end of the round – the score keeps changing until the last second. And look at a plain, high-contrast background: the sky or a wall rather than colourful ground.",
                      "请在回合结束时再按 – 分数会一直变动到最后一秒。同时请面向单调、对比明显的背景：天空或墙壁，而不是花花绿绿的地面。",
                      "ラウンドが終わってから押してください – スコアは最後の瞬間まで変わります。その際、空や壁など無地でコントラストのはっきりした背景を向いてください。" } },
